...
class PreviewPageFactoryTest {
    ...
    fun GIVEN_kobweb_host_WHEN_create_THEN_the_page_is_reached_through_a_route() {
        assertEquals(listOf("PreviewPage.kt", "KaleidosRoute.kt"), fileNames(DevServerKind.KOBWEB))

        val route = factory.create(plan(DevServerKind.KOBWEB))
            .first { file -> file.relativePath.endsWith("KaleidosRoute.kt") }
    ...
    fun GIVEN_modules_with_previews_WHEN_create_THEN_the_page_asks_every_registry() {
        ...
        assertTrue(
            content.contains("    if (fqn in kaleidos.generated.m_components_ui.PreviewRegistry.fqns) {"),
            content
        )
        assertTrue(
            content.contains("    kaleidos.generated.m_components_ui.PreviewRegistry.fqns"),
            content
        )
    }
